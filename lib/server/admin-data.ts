...
> = {
  routes: {
    select: `SELECT r.route_id "id", r.route_name "name", r.origin "origin", r.destination "destination",
      r.base_fare "baseFare", (SELECT COUNT(*) FROM smartmove_database.trips t WHERE t.route_id=r.route_id) "tripCount"
      FROM smartmove_database.routes r`,
  ...
  trips: {
    select: `SELECT t.trip_id "id", t.route_id "routeId", r.route_name "routeName", r.origin "origin", r.destination "destination",
      ...
      (SELECT COUNT(*) FROM smartmove_database.bookings b WHERE b.trip_id=t.trip_id) "bookingCount"
      FROM smartmove_database.trips t JOIN smartmove_database.routes r ON r.route_id=t.route_id
      JOIN smartmove_database.vehicles v ON v.vehicle_id=t.vehicle_id
      JOIN smartmove_database.drivers d ON d.driver_id=t.driver_id`,
  ...
  vehicles: {
    select: `SELECT v.vehicle_id "id", v.registration_number "registrationNumber", v.vehicle_type "vehicleType",
      ...
      (SELECT COUNT(*) FROM smartmove_database.trips t WHERE t.vehicle_id=v.vehicle_id) "tripCount"
      FROM smartmove_database.vehicles v`,
  ...
  drivers: {
    select: `SELECT d.driver_id "id", d.full_name "name", d.phone "phone", d.licence_number "licenceNumber",
      (SELECT COUNT(*) FROM smartmove_database.trips t WHERE t.driver_id=d.driver_id) "tripCount"
      FROM smartmove_database.drivers d`,
  ...
  passengers: {
    select: `SELECT p.passenger_id "id", p.full_name "name", p.phone "phone", u.email "email",
      (SELECT COUNT(*) FROM smartmove_database.bookings b WHERE b.passenger_id=p.passenger_id) "bookingCount"
      FROM smartmove_database.passengers p JOIN smartmove_database.app_users u ON u.user_id=p.user_id`,
  ...
  maintenance: {
    select: `SELECT m.maintenance_id "id", m.vehicle_id "vehicleId", v.registration_number "vehicle",
      ...
      FROM smartmove_database.maintenance_records m JOIN smartmove_database.vehicles v ON v.vehicle_id=m.vehicle_id`,
  ...
  bookings: {
    select: `SELECT b.booking_id "id", b.passenger_id "passengerId", p.full_name "passenger", b.trip_id "tripId",
      ...
      (SELECT COUNT(*) FROM smartmove_database.tickets tk WHERE tk.booking_id=b.booking_id) "seatCount",
      (SELECT LISTAGG(tk.seat_number,', ') WITHIN GROUP (ORDER BY tk.seat_number) FROM smartmove_database.tickets tk WHERE tk.booking_id=b.booking_id) "seats",
      (SELECT LISTAGG('#' || tk.ticket_id || ' seat ' || tk.seat_number || ' ' || tk.status, ', ')
        WITHIN GROUP (ORDER BY tk.seat_number) FROM smartmove_database.tickets tk WHERE tk.booking_id=b.booking_id) "tickets",
      (SELECT NVL(SUM(tk.fare_amount),0) FROM smartmove_database.tickets tk WHERE tk.booking_id=b.booking_id) "totalFare",
      py.payment_id "paymentId", py.amount "paidAmount", rf.amount "refundAmount"
      FROM smartmove_database.bookings b JOIN smartmove_database.passengers p ON p.passenger_id=b.passenger_id
      JOIN smartmove_database.trips t ON t.trip_id=b.trip_id JOIN smartmove_database.routes r ON r.route_id=t.route_id
      LEFT JOIN smartmove_database.payments py ON py.booking_id=b.booking_id
      LEFT JOIN smartmove_database.refunds rf ON rf.payment_id=py.payment_id`,
  ...
  refunds: {
    select: `SELECT b.booking_id "id", p.full_name "passenger", py.payment_id "paymentId", py.amount "amount",
      ...
      FROM smartmove_database.bookings b JOIN smartmove_database.passengers p ON p.passenger_id=b.passenger_id
      JOIN smartmove_database.payments py ON py.booking_id=b.booking_id
      JOIN smartmove_database.trips t ON t.trip_id=b.trip_id JOIN smartmove_database.routes r ON r.route_id=t.route_id
      LEFT JOIN smartmove_database.refunds rf ON rf.payment_id=py.payment_id`,
...
async function loadAdminFeedback(
  ...
  const result = await connection.execute<{
    ...
    `SELECT f.feedback_id "id", f.booking_id "bookingId", f.feedback_type "type", f.status "status",
      ...
      FROM smartmove_database.feedback_records f JOIN smartmove_database.bookings b ON b.booking_id=f.booking_id
      JOIN smartmove_database.passengers p ON p.passenger_id=b.passenger_id
      JOIN smartmove_database.trips t ON t.trip_id=b.trip_id JOIN smartmove_database.routes r ON r.route_id=t.route_id
...
export async function loadAdminSummary(connection: Connection) {
  const result = await connection.execute<{
    ...
    `SELECT
      (SELECT COUNT(*) FROM smartmove_database.trips WHERE status='SCHEDULED' AND departure_at>SYSDATE) "upcomingTrips",
      (SELECT COUNT(*) FROM smartmove_database.bookings WHERE booked_at>=TRUNC(SYSDATE,'MM')
        AND booked_at<ADD_MONTHS(TRUNC(SYSDATE,'MM'),1)) "bookingsThisMonth",
      (SELECT NVL(SUM(amount),0) FROM smartmove_database.payments WHERE paid_at>=TRUNC(SYSDATE,'MM')
        AND paid_at<ADD_MONTHS(TRUNC(SYSDATE,'MM'),1)) "grossRevenue",
      (SELECT COUNT(*) FROM smartmove_database.maintenance_records WHERE status='SCHEDULED'
        AND scheduled_date<TRUNC(SYSDATE)+8) "maintenanceDue",
      (SELECT COUNT(*) FROM smartmove_database.tickets tk JOIN smartmove_database.bookings b ON b.booking_id=tk.booking_id
        JOIN smartmove_database.trips t ON t.trip_id=b.trip_id WHERE t.status='SCHEDULED'
      ...
      (SELECT NVL(SUM(v.seat_count),0) FROM smartmove_database.trips t
        JOIN smartmove_database.vehicles v ON v.vehicle_id=t.vehicle_id WHERE t.status='SCHEDULED'
